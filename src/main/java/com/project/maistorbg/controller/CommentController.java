...
public class CommentController extends MyExceptionHandler{
    ...
    public ResponseEntity<List<CommentInfoDTO>> getAllCommentsBySender(HttpSession session) {
    ...
    public ResponseEntity<CommentInfoDTO> addComment (@Valid @RequestBody AddCommentDTO dto, HttpSession s){
        if(s.getAttribute("LOGGED_ID") == null){
            return ResponseEntity.status(HttpStatus.UNAUTHORIZED).body(null);
        }
        int id = (int) s.getAttribute("LOGGED_ID");
        CommentInfoDTO addedComment = commentService.add(dto,id);
        return ResponseEntity.status(HttpStatus.CREATED).body(addedComment);
    }

    @PutMapping("/comments/{commentId}")
    public ResponseEntity<CommentInfoDTO> editComment(@PathVariable int commentId, @Valid @RequestBody EditCommentDTO dto, HttpSession s) {
        if (s.getAttribute("LOGGED_ID") == null) {
            return ResponseEntity.status(HttpStatus.UNAUTHORIZED).body(null);
        }
